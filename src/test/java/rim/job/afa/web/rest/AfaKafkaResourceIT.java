...
import rim.job.afa.config.KafkaProperties;

class AfaKafkaResourceIT {
    ...
    void setup() {
        ...
        AfaKafkaResource kafkaResource = new AfaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/afa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/afa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
